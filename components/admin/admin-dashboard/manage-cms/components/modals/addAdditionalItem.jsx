...
const AddAdditionalItemModal = ({ show, onClose, field = {} }) => {
  ...
  console.log("Field Data in Blog Modal iiiiii:", field);
  ...
  const [formData, setFormData] = useState({
    ...
    images: [],
    oldImages: field.image || [],      // existing URLs
  ...
  const [imagePreviewLink, setImagePreviewLink] = useState(
    // field.image?.length ? field.image : []
    Array.isArray(field.image) ? field.image : field.image ? [field.image] : []
  );
  ...
  const handleImageChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      newImages: [file],     // always replace previous image
    }));

    setImagePreviewLink([previewURL]); // show only one preview
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Form Data Submitted User Blog By Chandra Sarkar:", formData);
    // return;
    setLoading(true);
    setError(null);
    setSuccess(null);
    ...
    try {
      ...
      if (!formData._id) {
        ...
        response = await axios.put(
          `${apiurl}/api/userblog/edit-cms/${formData._id}`,
          formDataToSend,
          {
            headers: {
              Authorization: `Bearer ${token}`,
          ...
          <div className="modal-content">
            ...
            <div className="modal-header">
              <h5 className="modal-title">
                {formData._id ? "Update CMS" : "Add CMS"}
              </h5>
              <button
                type="button"
                className="btn-close"
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="row">
                  ...
                  <div className="mb-3 col-md-6">
                    ...
                    />
                    {formErrors.summary && (
                      <div className="invalid-feedback">{formErrors.summary}</div>
                    )}
                  </div>
